...
const UserProfilePage = () => {
  ...
  const username = "santhoshkrishn"; // Static username

  useEffect(() => {
    // Fetch user posts
    axios.get(`http://localhost:3000/getposts/${username}`)
      .then(response => {
    ...
    axios.get(`http://localhost:3000/ecofund/${username}`)
      .then(response => {
        ...
        if (Array.isArray(response.data)) {
          ...
          response.data.forEach(donation => {
            if (donation.post_id) {
              axios.get(`http://localhost:3000/posts/${donation.post_id}`)
                .then(response => {
          ...
          console.error("Unexpected donations data structure:", response.data);
        }
      })
      .catch(error => {
  ...
  const postsToDisplay = showDonations
    ? posts.filter(post => donations.some(donation => donation.post_id === post._id))
    : posts;

  return (
    <div className="user-profile-page p-4 flex flex-col items-center bg-green-50">
      {/* User Profile Section */}
      <div className="user-info flex items-center mb-6 w-full max-w-2xl">
        <img
        ...
        <div className="ml-4">
          ...
          <a
            href="#"
            className="text-green-700 text-lg lg:text-xl"
          >
            {username}
          </a>
